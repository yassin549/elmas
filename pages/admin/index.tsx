...
const OrdersTable = ({ orders, isLoading, error }: { orders: Order[]; isLoading: boolean; error: any }) => {
  ...
  return (
    <div>
      ...
      <div className='md:hidden space-y-4'>
        {orders.map(order => (
          <div key={order.id} onClick={() => router.push(`/admin/orders/${order.id}`)} className='bg-gray-800/50 p-4 rounded-lg shadow-md cursor-pointer'>
            <div className='flex justify-between items-start mb-2'>
              <div>
                <p className='font-bold text-white'>{order.shipping ? `${order.shipping.firstName} ${order.shipping.lastName}` : 'N/A'}</p>
                <p className='text-xs text-gray-400'>{new Date(order.createdAt).toLocaleDateString()}</p>
              </div>
              <p className='font-mono text-xs text-gray-400'>#{order.id.substring(0, 8)}</p>
            </div>
            <div className='flex justify-between items-center text-sm'>
              <p className='font-medium text-gray-300'>${order.total.toFixed(2)}</p>
      ...
      <div className='hidden md:block overflow-x-auto'>
        ...
        <tbody className='divide-y divide-gray-100 dark:divide-gray-800'>
          {orders.map(order => (
            <tr key={order.id} onClick={() => router.push(`/admin/orders/${order.id}`)} className='hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors duration-200 cursor-pointer'>
              <td className='px-6 py-4 font-mono text-xs text-gray-700 dark:text-gray-400'>#{order.id.substring(0, 8)}</td>
              <td className='px-6 py-4 font-medium text-gray-900 dark:text-white'>{order.shipping ? `${order.shipping.firstName} ${order.shipping.lastName}` : 'N/A'}</td>
              ...
              <td className='px-6 py-4 text-gray-600 dark:text-gray-300'>
                {order.shipping ? `${order.shipping.address}, ${order.shipping.city}, ${order.shipping.postalCode}, ${order.shipping.country}` : 'N/A'}
              </td>
            </tr>
          ))}
        </tbody>
